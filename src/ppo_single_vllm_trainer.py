...
from src.rloo_single_vllm_trainer import vllm_generate
from src.utils import INVALID_LOGPROB, OnlineTrainerState, copy_to, prepare_deepspeed


@dataclass
class PPOVLLMConfig(PPOv2Config):
    sync: bool = True
...
class PPOSingleVLLMTrainer(Trainer):
    def __init__(
        self,
        config: PPOVLLMConfig,
    ...
    def train(self):
        ...
        for update in range(1, self.num_batches + 1):
            ...
            if accelerator.is_main_process:
                ...
                model_named_parameters = accelerator._get_named_parameters(model.policy)
                g_response_ids, _ = vllm_generate(
                    llm,
                    sampling_params,
            ...
            with torch.no_grad():
                ...
                for i in range(0, queries.shape[0], args.local_rollout_forward_batch_size):
                    ...
                    output = forward(model.policy, query_response, tokenizer.pad_token_id)
